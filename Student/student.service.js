...
const { count } = require("../Admin/admin.model");
...
async function student_login({ email, password, ipAddress }) {
  const account = await db.Student.findOne({
    email,
  });
...
//         std_id: id
//     }).populate("class_id").populate("std_id");
...
//             class_id: attendance[i].class_id.id,
...
//     var TotalAttendance = time.reduce(function (r, a) {

//         r[a.class_id] = r[a.class_id] || [];
//         r[a.class_id].present = 0
//         r[a.class_id].absent = 0

//         r[a.class_id].push(a);
//         return r;
//     }, Object.create(null));

//     var keys = Object.keys(TotalAttendance)
...
async function studentAttendaceAll(id, user) {
  ...
  for (i in TotalAttendance) {
    for (j = 0; j < TotalAttendance[i].length; j++) {
    ...
    //counter Present Absent Students 
    counterPr = 0;
    counterAb = 0;
